...
- (void)drawRect:(CGRect)rect
{
	...
	for ( unsigned int index = 0; index < [m_measureArray count]; index++ )
	{
        
        NSMeasure * measure = [m_measureArray objectAtIndex:index];
		...
		if ( index == ([m_measureArray count] - 1) )
		{
	...
	for ( unsigned int index = 0; index < [m_noteArray count]; index++ )
	{
        
        NSNote * note = [m_noteArray objectAtIndex:index];
