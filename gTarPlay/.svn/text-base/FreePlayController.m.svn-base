...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if ( self )
    {
        ...
        m_audioController.m_delegate = self;
...
- (void)dealloc
{
    ...
    m_audioController.m_delegate = nil;
...
- (void)viewDidLoad
{
    ...
    NSArray *ar = [m_audioController getInstrumentNames];
    NSMutableArray *instrumentScrollText = [ar mutableCopy];
    [instrumentScrollText insertObject:[NSString stringWithString:@"Guitars"]  atIndex:0];
    ...
    // set effects name
    m_effects = [m_audioController GetEffects];
...
{
    [m_audioController FretDown:fret onString:str - 1];
}

- (void)guitarFretUp:(GuitarFret)fret andString:(GuitarString)str
{
    [m_audioController FretUp:fret onString:str - 1];
}
...
{
    [m_audioController NoteOffAtString:str - 1 andFret:fret];
}
...
//    [m_activityIndicatorView stopAnimating];
    ...
    [m_guitarController setMinimumInterarrivalTime:0.05f];
...
- (void) turnONLED:(int)string AndFret:(int)fret WithColorRed:(int)red AndGreen:(int)green AndBlue:(int)blue
{
    ...
    [g_guitarController turnOnLedAtString:string
                                  andFret:fret
                                  withRed:red
                                 andGreen:green
                                  andBlue:blue];
    ...
    switch (m_LEDShape)
    {
        case LEDShapeCross:
            ...
            {
                [g_guitarController turnOnLedAtString:string+1
                                              andFret:fret
                                              withRed:red
                                             andGreen:green
                                              andBlue:blue];
...
{
    [g_guitarController turnOnLedAtString:0
                                  andFret:0
                                  withRed:0
                                 andGreen:0
                                  andBlue:0];
...
{
    [m_audioController SetBWCutoff:[m_toneSlider value]];
}
...
{
    [m_audioController setM_volumeGain:[m_lineOutVolumeSlider value]];
}
...
- (IBAction)instrumentSelected:(id)sender
{
    ...
    [m_audioController setSamplePackWithName:sampleName withSelector:@selector(samplerFinishedLoadingCB:) andOwner:self];
}
