...
- (void)initJamPad
{
	...
	NSString * filePath;
	filePath = [[NSBundle mainBundle] pathForResource:@"jam_pad_offlight" ofType:@"png"];  
	m_ledOff = [[UIImage alloc] initWithContentsOfFile:filePath];
	
	filePath = [[NSBundle mainBundle] pathForResource:@"jam_pad_onlight" ofType:@"png"];
	m_ledOn = [[UIImage alloc] initWithContentsOfFile:filePath];
...
- (void)turnOffLedWidth:(NSInteger)width andHeight:(NSInteger)height
{
	...
	{
		UIImageView * image = m_ledOnGrid[height][width];
...
-(void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
	if ( m_delegate != nil )
	{
		[m_delegate jamPad:self positionChanged:m_currentDiscretizedPosition];
	}
}
