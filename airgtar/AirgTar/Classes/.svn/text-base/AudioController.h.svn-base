...
public:
	...
	void Pluck(float freq)
	{
		...
		m_FreqKS = freq;
		m_BufferKS_n = (int) m_Fs / m_FreqKS;
		...
		m_eBufferKS = m_BufferKS_n - 1;
		
		srand( time(NULL) );
		
		// Now initialize the delay line buffer with noise
		for(int i = 0; i < m_BufferKS_n; i++)
		{
	...
	void Kill()
	{
		...
		if(m_BufferKS != NULL && m_BufferKS_n > 0)
			memset(m_BufferKS, 0, m_BufferKS_n * sizeof(SInt16));
	}
	...
	SInt16 GetNextKSSample()
	{
		...
		if(m_eBufferKS + 1 >= m_BufferKS_n)
			B = m_BufferKS[0];
		else 
			B = m_BufferKS[m_eBufferKS + 1];
		
		/*
		if(m_eBufferKS + 2 >= m_BufferKS_n)
			C = m_BufferKS[m_eBufferKS + 2 - m_BufferKS_n - 1];
		else 
...
public:
	...
	static int GuitarMidiLookup(int string, int fret)
	{
	...
	// This is actually capable of taking in negative fret positions if needed!
