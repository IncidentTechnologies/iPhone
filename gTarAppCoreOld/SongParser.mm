	...
	attributes:(NSDictionary *)attributeDict
{
	...
	{
		m_currentSong->m_tempo = [[attributeDict objectForKey:@"value"] intValue];
	}	
	else if ([elementName isEqualToString:@"guitarposition"])
	{
        m_currentNote->m_string = [[attributeDict objectForKey:@"string"] intValue];
        
        NSString * fretValue = [attributeDict objectForKey:@"fret"];
        
        // Check if this is not a valid integer
        if ( [fretValue isEqualToString:@"0"] == NO && 
             [fretValue intValue] == 0 )
        {
            // this is a muted noted
            m_currentNote->m_fret = GTAR_GUITAR_FRET_MUTED;
        }
        else
        {
            m_currentNote->m_fret = [fretValue intValue];
        }
...
//		CGFloat endBeat = beatCount + startBeatCount;
	...
	{
		const char * text = [m_currentText cStringUsingEncoding:NSASCIIStringEncoding];
