...
public:
	...
	int ReadSerialPort(char * &n_pBuffer)
	{
		if (m_fd == -1) 
			return 0;
		
		n_pBuffer = new char[MAX_BUFFER_SIZE];
		int bytes_n = 0;
	...
	int SendLEDMsg(int string, int fret, char *pszOnOff)
	{
		if(m_fd == -1)
			return 0;
	...
	int WriteSerialPort(char *psz, int psz_n)
	{
