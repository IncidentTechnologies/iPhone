...
- (CSong*)convertToCSong
{
	...
	// Copy over all the metadata for the song
	song->m_id = m_id;
	song->m_tempo = m_tempo;
	
	song->SetName( [m_title cStringUsingEncoding:NSASCIIStringEncoding] );
	song->SetArtist( [m_author cStringUsingEncoding:NSASCIIStringEncoding] );
	song->SetDescription( [m_description cStringUsingEncoding:NSASCIIStringEncoding] );
    song->SetInstrument( [m_instrument cStringUsingEncoding:NSASCIIStringEncoding] );
    
	// Copy over all the measures
	for ( unsigned int index = 0; index < [m_measures count]; index++ )
	{
		NSMeasure * nsMeasure = [m_measures objectAtIndex:index];
		
		CMeasure * cMeasure = [nsMeasure convertToCMeasure];
		
		song->AddMeasure( cMeasure );
