...
- (CMeasure*)convertToCMeasure
{
	...
	measure->m_startBeat = m_startBeat;
	measure->m_beatCount = m_beatCount;
	measure->m_beatValue = m_beatValue;
	
	for ( unsigned int index = 0; index < [m_notes count]; index++ )
	{
		
		NSNote * nsNote = [m_notes objectAtIndex:index];
		
		CNote * cNote = [nsNote convertToCNote];
		
		measure->AddNote( cNote );
