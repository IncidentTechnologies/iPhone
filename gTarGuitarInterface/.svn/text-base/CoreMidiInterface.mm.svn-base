...
CoreMidiInterface::CoreMidiInterface(MidiCallback fnCallback, MidiConnectionChangeCallback fnConnectionChangeCallback, void *pContext) :
    m_pMidiClient(NULL),
    m_pSources(NULL),
...
//    m_fnStringOut(NULL)
{
    ...
    m_pSendQueue = new std::list<MIDISysexSendRequest*>();
    
    NSLog(@"CoreMidiInterface");
}
...
CoreMidiInterface::~CoreMidiInterface()
{
    ...
    if(m_pMidiClient != NULL)
    {
        ...
        if( oss != 0 )
            NSLog(@"Failed to dispose client: Error %ld", oss);
    ...
    if(m_pSendQueue != NULL)
    {
        ...
        for(std::list<MIDISysexSendRequest*>::iterator req = m_pSendQueue->begin(); req != m_pSendQueue->end(); req++)
        {
            
            MIDISysexSendRequest *pSendReq = (*req);
...
void CoreMidiInterface::UpdateEndpoints()
{
    ...
    NSLog(@"CoreMidiInterface:UpdateEndpoints:Found %d sources and %d destinations", src_n, dest_n);
    ...
    if(connected == true)
    {
        if(IsGtarConnected() == false)
        {
            ...
            NSLog(@"CoreMidiInterface:UpdateEndpoints:gTar Disconnected");
        }
    }
    else
    {
        if(IsGtarConnected() == true)
        {
            ...
            NSLog(@"CoreMidiInterface:UpdateEndpoints:gTar Connected");
        }
    }
...
ItemCount CoreMidiInterface::UpdateSources()
{
    ...
    for(ItemCount i = 0; i < Sources_n; i++)
    {
        ...
        if(pSourceEndpoint != NULL)
        {
            ...
            if( oss != 0 )
                NSLog(@"Failed to connect source: Error %ld", oss);            
    ...
    for(ItemCount i = 0; i < Destinations_n; i++)
    {
        ...
        if(pDestinationEndpoint != NULL)
        {
            ...
            if([((NSString*)name) isEqualToString:@"gTar"] == YES)
            {
                ...
                NSLog(@"gTar destination connected");
            }
...
void CoreMidiInterface::init()
{
    ...
    if( oss != 0 )
        NSLog(@"Failed to create AE Midi Client: Error %ld", oss);
    ...
    if( oss != 0 )
        NSLog(@"Failed to create input port: Error %ld", oss);

    // Set up the Output Port
    oss = MIDIOutputPortCreate(m_pMidiClient, (CFStringRef)@"MidiClient Output Port", &m_pMidiOutputPort);
    
    if( oss != 0 )
        NSLog(@"Failed to create output port: Error %ld", oss);
    
    
    m_pSendLock = [[NSLock alloc] init];
...
{
    int src_n = m_pSources->size();
    int dest_n = m_pDestinations->size();
    ...
    switch(mid)
    {
        ...
        case kMIDIMsgObjectAdded:
        {
            ...
            if(pMidAdd->childType == kMIDIObjectType_Source)            
                NSLog(@"Source Object Added");
            else if(pMidAdd->childType == kMIDIObjectType_Destination)
                NSLog(@"Destination Object Added");
        ...
        case kMIDIMsgObjectRemoved:
        {
            ...
            if(pMidRemoved->childType == kMIDIObjectType_Source)            
                NSLog(@"Source Object Removed");
            else if(pMidRemoved->childType == kMIDIObjectType_Destination)
                NSLog(@"Destination Object Removed");
    ...
    for(int i = 0; i < pPacketList->numPackets; i++)
    {
        ...
        */
        ...
        if(pkt.length == 3)
            ...
            NSLog(@"Invalid Midi packet size: %u", pkt.length);
    }
}
...
RESULT CoreMidiInterface::SendBuffer(unsigned char *pBuffer, int pBuffer_n)
{
    ...
    for(std::list<MIDIObjectRef>::iterator it = m_pDestinations->begin(); it != m_pDestinations->end(); it++)
    {        
        MIDIEndpointRef ep = reinterpret_cast<MIDIEndpointRef>(*it);    
        OSStatus oss = MIDISend(m_pMidiOutputPort, ep, &PacketList);    
        CBRM((oss != -1), @"SendBuffer: MIDISend failed with status 0x%x", oss);
    }
...
static void MIDICompletionHander(MIDISysexSendRequest *request)
{
    ...
    [pInterface->m_pSendLock lock];
    
    MIDISysexSendRequest *pSendReq = pInterface->m_pSendQueue->front();
    pInterface->m_pSendQueue->pop_front();    
    delete pSendReq;
    
    // see if we have anything else queued up
    if(pInterface->m_pSendQueue->size() > 0)
    {
        pSendReq = pInterface->m_pSendQueue->front();
        
        OSStatus oss = MIDISendSysex(pSendReq);
        CBRM((oss != -1), @"SendSysExBuffer: MIDISend failed with status 0x%x", oss);
    }
    
Error:
    [pInterface->m_pSendLock unlock];
...
RESULT CoreMidiInterface::SendSysExBuffer(unsigned char *pBuffer, int pBuffer_n)
{
    ...
    {    
        ...
        pSendReq->destination = reinterpret_cast<MIDIEndpointRef>(*it);    
        ...
        m_pSendQueue->push_back(pSendReq);
        
        // If nothing else is on the queue, send this now
        if ( m_pSendQueue->size() == 1 )
        {
            OSStatus oss = MIDISendSysex(pSendReq);
            CBRM((oss != -1), @"SendSysExBuffer: MIDISend failed with status 0x%x", oss);
        }
...
RESULT CoreMidiInterface::SendSetNoteActive(unsigned char R, unsigned char G, unsigned char B)
{
    ...
    CRM_NA(SendSysExBuffer(pSendBuffer, pSendBuffer_n), @"SendSetLEDState: Failed to send SysEx Buffer");
...
RESULT CoreMidiInterface::SendRequestCertDownload() {
    ...
    CRM_NA(SendSysExBuffer(pSendBuffer, pSendBuffer_n), @"SendRequestCertDownload: Failed to send SysEx Buffer");
...
RESULT CoreMidiInterface::SendRequestBatteryStatus() {
    RESULT r = R_NO_ISSUE;
    ...
    pSendBuffer[0] = 0xF0;           // SysEx Message
    pSendBuffer[1] = GTAR_DEVICE_ID;
    pSendBuffer[2] = (unsigned char)GTAR_MSG_REQ_BAT_STATUS;
    //pSendBuffer[4] = 0xF7;           // End SysEx Message  
    
    CRM_NA(SendSysExBuffer(pSendBuffer, pSendBuffer_n), @"SendRequestBatteryStatus: Failed to send SysEx Buffer");
...
RESULT CoreMidiInterface::SendEnableDebug() {
    RESULT r = R_NO_ISSUE;
    ...
    pSendBuffer[0] = 0xF0;           // SysEx Message
    pSendBuffer[1] = GTAR_DEVICE_ID;
    pSendBuffer[2] = (unsigned char)GTAR_MSG_DBG_ENABLE;
    //pSendBuffer[4] = 0xF7;           // End SysEx Message  
    
    CRM_NA(SendSysExBuffer(pSendBuffer, pSendBuffer_n), @"SendEnableDebug: Failed to send SysEx Buffer");
...
RESULT CoreMidiInterface::SendDisableDebug() {
    RESULT r = R_NO_ISSUE;
    ...
    pSendBuffer[0] = 0xF0;           // SysEx Message
    pSendBuffer[1] = GTAR_DEVICE_ID;
    pSendBuffer[2] = (unsigned char)GTAR_MSG_DBG_DISABLE;
    //pSendBuffer[4] = 0xF7;           // End SysEx Message  
    
    CRM_NA(SendSysExBuffer(pSendBuffer, pSendBuffer_n), @"SendDisableDebug: Failed to send SysEx Buffer");
...
RESULT CoreMidiInterface::SendRequestFirmwareVersion() {
    ...
    CRM_NA(SendSysExBuffer(pSendBuffer, pSendBuffer_n), @"SendRequestFirmwareVersion: Failed to send SysEx Buffer");
...
RESULT CoreMidiInterface::SendFirmwarePackagePage(unsigned char *pBuffer, int pBuffer_n, int totSize, int totPages, int curPage, unsigned char checkSum)
{
    ...
    CRM_NA(SendSysExBuffer(pSendBuffer, pSendBuffer_n), @"SendFirmwarePackagePage: Failed to send SysEx Buffer");
...
RESULT CoreMidiInterface::SendCCSetLEDState(unsigned char fret, unsigned char str, unsigned char R, unsigned char G, unsigned char B, unsigned char M) {
    ...
    CRM_NA(SendBuffer(sendBuffer, 3), @"SendCCSetLEDState: Failed to send Buffer");
...
RESULT CoreMidiInterface::SendNoteMsg(unsigned char channel, unsigned char midiVal, unsigned char velocity, const char* pszOnOff)
{
    ...
    CRM_NA(SendBuffer(sendBuffer, 3), @"SendNoteMsg: Failed to send Buffer");
