...
void DeviceInterface::ConsolidateBufferBytes( int startIndex, int bytesToConsolidate )
{
	for ( unsigned int i = 0; i < bytesToConsolidate; i++ )
	{
...
void DeviceInterface::ReceiveDeviceOutput( char *fretsDown, char *notesOn )
{
	
	unsigned int currentIndex;
	unsigned int bytesRead = ReadSerialPort( m_buffer + m_bufferLength, DEVICE_INTERFACE_MAX_BUFFER_SIZE - m_bufferLength );
...
//				unsigned char velocity = m_buffer[currentIndex + 2];  
				...
				if ( notesOn[str] == fret )
				{
					notesOn[str] = DEVICE_INTERFACE_GUITAR_NOTE_OFF;
				}
...
//				unsigned char velocity = m_buffer[currentIndex + 2];  
			...
			case 0xB0:
			{
				...
				{
					// Fret up
					fretsDown[str] = DEVICE_INTERFACE_GUITAR_NOTE_OFF;
				}
...
bool DeviceInterface::TurnOnLED( int str, int fret )
{
	return SendLEDMsg( str, fret, (char*)"on");
}

bool DeviceInterface::TurnOffLED( int str, int fret )
{
	return SendLEDMsg( str, fret, (char*)"off");	
}

bool DeviceInterface::SendLEDMsg(int string, int fret, char *pszOnOff)
{
	if(m_fd == -1)
		return false;
	
	char tempString[DEVICE_INTERFACE_MAX_BUFFER_SIZE];
...
bool DeviceInterface::TurnOnLEDColor( int str, int fret, char red, char green, char blue )
{
	char colorString[] = "      ";
...
bool DeviceInterface::SendLEDColorMsg(int string, int fret, char *pszColor)
{
	if(m_fd == -1)
		return false;
...
bool DeviceInterface::TurnOnStateFretFollow(char red, char green, char blue)
{
	char colorString[] = "on       ";
	...
	return SendStateMsg( (char*)"fretfollow", colorString );
}

bool DeviceInterface::TurnOffStateFretFollow()
{
	return SendStateMsg( (char*)"fretfollow", (char*)"off" );
}

bool DeviceInterface::TurnOnStateNoteActive(char red, char green, char blue)
{
	char colorString[] = "on       ";
	...
	return SendStateMsg( (char*)"noteactive", colorString );
}

bool DeviceInterface::TurnOffStateNoteActive()
{
	return SendStateMsg( (char*)"noteactive", (char*)"off" );
}

bool DeviceInterface::TurnOnStateLightning(char red, char green, char blue)
{
	char colorString[] = "on       ";
	...
	return SendStateMsg( (char*)"lightning", colorString );
}

bool DeviceInterface::TurnOffStateLightning()
{
	return SendStateMsg( (char*)"lightning", (char*)"off" );
}

bool DeviceInterface::SendStateMsg(char *pszState, char *pszOnOffColor)
{
	if(m_fd == -1)
		return false;
...
int DeviceInterface::ReadSerialPort(unsigned char * pBuffer, unsigned int pBuffer_n)
{
	if (m_fd == -1) 
		return 0;
...
int DeviceInterface::ReadSerialPort(unsigned char * &n_pBuffer)
{
	if (m_fd == -1) 
		return 0;
	
	n_pBuffer = new unsigned char[DEVICE_INTERFACE_MAX_BUFFER_SIZE];
	int bytes_n = 0;
	
	bytes_n = read(m_fd, n_pBuffer, DEVICE_INTERFACE_MAX_BUFFER_SIZE);
	
	return bytes_n;
}

int DeviceInterface::WriteSerialPort(unsigned char *psz, int psz_n)
{
...
int DeviceInterface::OpenSerialPort()
{
	m_fd = -1;
...
void DeviceInterface::CloseSerialPort()
{
	if(m_fd != -1)
	{
		close(m_fd);
