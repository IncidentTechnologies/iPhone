...
- (void)getSampleListForCallbackObj:(id)callbackObj selector:(SEL)selector isCustom:(BOOL)isCustom
{
    ...
    NSInteger userId = 1; //(isCustom) ? g_loggedInUser.m_userId : 1;
    
    [g_ophoCloudController requestGetXmpFolderContentList:folderId andXmpType:OphoXmpTypeXMPSample andExcludeType:0 andUserId:userId andCallbackObj:callbackObj andCallbackSel:selector];
}
...
- (void)getInstrumentListForCallbackObj:(id)callbackObj selector:(SEL)selector isCustom:(BOOL)isCustom
{
    ...
    [g_ophoCloudController requestGetXmpFolderContentList:folderId andXmpType:OphoXmpTypeXMPInstrument andExcludeType:0 andUserId:userId andCallbackObj:callbackObj andCallbackSel:selector];
}
...
{
    [g_ophoCloudController requestGetXmpWithId:xmpId isXmpOnly:false andCallbackObj:callbackObj andCallbackSel:selector];
}
...
- (void)deleteWithId:(NSInteger)xmpId
{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
        [g_ophoCloudController requestDeleteXmpWithId:xmpId andCallbackObj:self andCallbackSel:@selector(deleteCallback:)];
    });
}
...
- (void)buildSortedXmpList:(NSArray *)xmpList withIds:(NSMutableArray *)fileIdSet withData:(NSMutableArray *)fileLoadSet withDates:(NSMutableArray *)fileDateSet withVersion:(NSMutableArray *)fileVersionSet withCustom:(NSMutableArray *)fileCustomSet;
{
    ...
    for(XmlDom * xmp in xmpList){
        ...
        NSNumber * custom = [NSNumber numberWithBool:false];
